...
def test():
    db_noise = np.linspace(-85, -75, 11)  # dB noise
    ...
    for ii in range(len(db_noise)):
        ...
        lin_noise = 10 ** ((np.random.randn(len(beam3_tri[2, :])) + db_noise[ii]) / 20)
        beam3_tri[2, :] += lin_noise
        lin_noise = 10 ** ((np.random.randn(len(beam2_tri[2, :])) + db_noise[ii]) / 20)
        beam2_tri[2, :] += lin_noise
        lin_noise = 10 ** ((np.random.randn(len(beam1_tri[2, :])) + db_noise[ii]) / 20)
        beam1_tri[2, :] += lin_noise
